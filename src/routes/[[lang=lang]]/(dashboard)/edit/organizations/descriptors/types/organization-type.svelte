<script lang="ts">
	...
	const organizationTypeForm = extendedSuperForm(organizationTypeFormData, {
		dataType: 'json',
		resetForm: false,
	});

	const { form, constraints } = organizationTypeForm;

	let deleteRef = $state<HTMLButtonElement>();
</script>

<DescriptorFormDialog form={organizationTypeForm} action="?/update">
