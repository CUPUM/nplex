<script lang="ts">
	...
	const expertiseForm = extendedSuperForm(organizationExpertiseFormData, {
		dataType: 'json',
		resetForm: false,
	});

	const { form, constraints } = expertiseForm;

	let deleteRef = $state<HTMLButtonElement>();

	const dialog = new DialogForm(expertiseForm);
</script>
