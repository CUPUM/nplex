    ...
    return (
        select json_build_object(
           ...
           'interventions', (
                ...
                from (
                    select i.*, coalesce(array_agg(ti.type) filter (where ti is not null), '{}') as types
            ...
            'siteUsages', (
                ...
                from (
                    select su.*, coalesce(array_agg(supc.category) filter (where supc is not null), '{}') as categories
            ...
            'eventTypes', (
                ...
                from (
                    select e.*, coalesce(array_agg(seby.subevent) filter (where seby is not null), '{}') as subevent  
            ...
            'exemplarityCategories', (
                ...
                from (
                    select ec.*, coalesce(array_agg(ei.*) filter (where ei is not null), '{}') as indicators
