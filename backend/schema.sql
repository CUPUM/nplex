...
comment on table public.profiles is 'Main table for projects.';
...
-- To do: add trigger to update projects 'ratings_n' and 'ratings_average' on update, insert, and delete.
...
create policy "Only editors and admins can intervene on self-publishable projects."
	...
	using (
		public.user_has_role('admin', 'editor')
	);
...
create policy "Authed users can only insert items using their uid and to existing collections they can select."
	...
	with check (
		...
		and exists (
			...
			and public.users_projects_collections.id = public.users_projects_collections_items.collection_id
		)
	);
